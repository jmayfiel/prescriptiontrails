...
?>
      {
        value: "<?php echo($trail['name']); ?>",
        label: "<?php echo($trail['name']); ?>",
		...
		if(totaldistance == 0 || missing === false) {
			...
			$.ajax({
				...
				.done(function(data) {
					...
					if(data.status == "done") {
						ga('send', 'event', "log", "log-"+tid, "steps: "+steps);
						$("#activitySubmit").hide();
						Materialize.toast("Trail logged!", 5000);
						$("#replace").html("<div class='row center-align' style='margin-top:50px;'><h1><i class='fa fa-2x fa-thumbs-up'></i></h1></div><div class='row'><div class='col s12 center-align'><p class='flow-text'>Your activity was logged successfully!</p></div></div>"+
